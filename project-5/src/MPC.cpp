...
double paramEpsi = 20;
double paramVel = 1.0;
double paramDelta = 200;
double paramAcceleration = 1.0;
double paramDeltaDiff = 200;
double paramAccDiff = 50.0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> phiDest = CppAD::atan(x0*(3*coeffs[3]*x0 + 2*coeffs[2]) + coeffs[1]);
      ...
      fg[1 + epsi_start + t] = epsi1 - (phiDest + ((v0 / Lf) * delta0 * dt));
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = static_cast<size_t>(6 * N + 2 * (N - 1));
  ...
  for(int i = 0; i < N; ++i) {
    xPred.push_back(solution.x[x_start + i]);
    yPred.push_back(solution.x[y_start + i]);
  }
